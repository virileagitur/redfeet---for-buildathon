...
const postPrompts = {
  "p1": ["kyoto%20bamboo%20grove%20photography", "kyoto%20bamboo%20forest%20sunlight", "arashiyama%20bamboo%20path", "japan%20kyoto%20temple%20nature"],
...
const newContent = content.replace(/id:\s*"([^"]+)",[\s\S]*?images:\s*\[([^\]]+)\]/g, (match, id) => {
  ...
  // Use WSVR CDN proxy to cache pollinations image and prevent timeouts
  const imagesArrayStr = prompts.map((p, i) => `"https://wsrv.nl/?url=https://image.pollinations.ai/prompt/${p}%20realistic%20photography?width=1000&height=1000&seed=${i + 1}&nologo=true"`).join(',\n      ');
...
console.log(`Updated ${count} posts with CDN cached semantic images!`);
